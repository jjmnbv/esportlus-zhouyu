 ...
 * @description: 内部支付-暴鸡币支付生产者
 * @author: xusisi
 * @create: 2018-11-03 17:20
 **/
@MQTransactionProducer(producerGroup = RocketMQConstant.EXTERNAL_GCOIN_PAY_PRODUCER_GROUP)
public class WalletPayProducer extends AbstractMQTransactionProducer {

    private static Logger logger = LoggerFactory.getLogger(WalletPayProducer.class);
    ...
    public LocalTransactionState executeLocalTransaction(Message msg, Object arg) {
        if (arg instanceof MQSimpleOrder) {
            logger.info("内部支付-暴鸡币支付更新订单消息入列成功");
            return LocalTransactionState.COMMIT_MESSAGE;
        }
        logger.info("内部支付-暴鸡币支付更新订单消息入列异常");
