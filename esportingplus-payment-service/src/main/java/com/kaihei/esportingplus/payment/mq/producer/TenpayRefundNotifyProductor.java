 ...
 * @description: 微信、QQ支付，退款回调生产者
 * @author: xusisi
 * @create: 2018-11-03 17:20
 **/
@MQTransactionProducer(producerGroup = RocketMQConstant.EXTERNAL_TENPAY_REFUND_NOTIFY_PRODUCER_GROUP)
public class TenpayRefundNotifyProductor extends AbstractMQTransactionProducer {

    private static Logger logger = LoggerFactory.getLogger(TenpayRefundNotifyProductor.class);
    ...
    public LocalTransactionState executeLocalTransaction(Message msg, Object arg) {
        if (arg instanceof ExternalRefundOrderMQ) {
            logger.info("微信、QQ支付,退款回调消息入列成功");
            return LocalTransactionState.COMMIT_MESSAGE;
        }
        logger.info("微信、QQ支付,退款回调消息入列异常");
