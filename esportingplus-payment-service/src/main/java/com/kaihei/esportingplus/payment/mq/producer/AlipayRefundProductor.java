 ...
 * @description: 支付宝退款生产者
 * @author: xusisi
 * @create: 2018-11-03 17:20
 **/
@MQTransactionProducer(producerGroup = RocketMQConstant.EXTERNAL_ALIPAY_REFUND_PRODUCER_GROUP)
public class AlipayRefundProductor extends AbstractMQTransactionProducer {

    private static Logger logger = LoggerFactory.getLogger(AlipayRefundProductor.class);
    ...
    public LocalTransactionState executeLocalTransaction(Message msg, Object arg) {
        if (arg instanceof ExternalRefundOrderMQ) {
            logger.info("用户发起支付宝退款消息入列成功");
            return LocalTransactionState.COMMIT_MESSAGE;
        }
        logger.info("用户发起支付宝退款消息入列异常");
